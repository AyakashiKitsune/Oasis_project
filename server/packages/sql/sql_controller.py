...
from ..models.Inventory_Analysis_model import Inventory_Analysis
...
class Database:
        ...
        if not Database.isrun:
            ...
            Base.metadata.create_all(
                ...
                tables=[
                    ...
                    Inventory_Analysis.__table__,  # child
                ],  # type: ignore
            )
            Database.isrun = True
    ...
    def importTableOasisBaseSales(self, columns):
        ...
        # read the old table
        df = pd.read_sql_table(table_name="original_table",
                               con=self.engine.connect())
        ...
        for i in diffdate:
            self.insert(Sales(date=f"{i.year}-{i.month}-{i.day}"))
    ...
    def get_null_rows(self, df):
        ...
        for col in null_clms:
            tables.append(
                {"column_name": col, "table": df[df[col].isnull()].to_dict()})
        print(tables)
        del null_clms
    ...
    def readSalesRecent(self, wholesale=False):
        ...
        if wholesale:
            ...
            request = self.session.execute(
                select(Sales).where(Sales.date ==
                                    recentdate).order_by(asc(Sales.date))
            ).scalars()
            return [i.to_dict() for i in request]
    ...
    def overview_query(
        ...
        fourteen_days_wholesales = self.readSalesBetweendates(
            fromdate=fromdate, todate=recentdate, wholesale=True
        )
        seven_days_wholesales = fourteen_days_wholesales[7:]
        total_sales_year = self.session.execute(
            select(func.sum(Sales.sale)).where(
                func.year(Sales.date) == currentDate.date
            )
        ).scalar()
        total_sold_year = self.session.execute(
            select(func.count(Sales.sale)).where(
                func.year(Sales.date) == currentDate.date
            )
        ).scalar()
        sold_count_product = self.session.execute(
            ...
            .group_by(Sales.name).order_by(desc("sold"))
        ).fetchall()

        sold_count_product = [{"name": i[0], "sold": i[1]}
                              for i in sold_count_product]
        if total_sales_year == None:
            total_sales_year = 0
        return (
            fourteen_days_wholesales,
            seven_days_wholesales,
            sold_count_product,
            total_sales_year,
            total_sold_year,
        )
    ...
    def recent_date(self):
        res = self.session.execute(select(func.max(Sales.date))).scalar()
        res1 = self.session.execute(select(func.min(Sales.date))).scalar()
        return res, res1

    def distinctValuesColumn(self):
        return self.session.execute(select(Sales.name).distinct()).scalars().all()
    ...
    def makesavekilltable(self):
        ...
        products.remove("None")  # type: ignore
        odf = pd.DataFrame(
            columns=["name" ,'january', 'february', 'march', 'april','may', 'june', 'july', 'august','september', 'october', 'november', 'december', ]
        )
        for product in products:
            fetch = self.session.execute(
                select(func.month(Sales.date), func.count(Sales.sale))
                .where(Sales.name == product)
                .group_by(func.month(Sales.date))
            ).fetchall()
            arg = np.median([i[1] for i in fetch])
            months = {
                f"{SaveKill.month_dict[i[0]]}": 1 if i[1] >= arg else 0 for i in fetch
            }
            json = {"name": product, **months}
            odf.loc[len(odf)] = json
        odf.replace(np.nan,0, inplace=True)
        odf.to_sql(
            name=SaveKill.__tablename__,
            con=self.engine.connect(),
            if_exists='replace',
            index=True,
            index_label='id'
    ...
    def makeInventoryAnalysis(self):
        products = (
            Database()
            .session.execute(select(Sales.name).distinct())
            .scalars()
            .fetchall()
        )
        products.remove("None")
        odf = pd.DataFrame(
            columns=["name", *[Inventory_Analysis.month_dict[i]
                               for i in range(1, 13)], "min", "max", "average"]
        )
        ...
        for product in products:
            res = (
                ...
                .session.execute(
                    select(
                        func.month(Sales.date), func.count(
                            Sales.name).label("count")
                    )
                    .where(Sales.name == product)
                    .group_by(func.month(Sales.date))
                )
            ...
            l = [product, *df["sold"].tolist(), min, max, average]
            odf.loc[len(odf)] = l
